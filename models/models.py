...
class FrameSyn(torch.nn.Module):
    def __init__(self, config, inpainter_pipeline, depth_model, vae, rotation,
        ...
        ] #COMMENT: seem useless, so no need to modify

        if config["custom_mode"] == "wonderjourney":
            x = torch.arange(512).float() + 0.5
            y = torch.arange(512).float() + 0.5
        elif config["custom_mode"] == "wonderjourney_576_1024":
            x = torch.arange(1024).float() + 0.5
            y = torch.arange(576).float() + 0.5
        else:
            raise("no implementation for given custom_mode")
    ...
    def get_init_camera(self):
        ...
        if self.config["custom_mode"] == "wonderjourney":
            K[0, 0, 2] = 256
            K[0, 1, 2] = 256
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            K[0, 0, 2] = 1024 / 2  # 修改光心坐标的 x 值为图像宽度的一半  
            K[0, 1, 2] = 576 / 2   # 修改光心坐标的 y 值为图像高度的一半 
        else:
            raise("no implementation for given custom_mode")
        K[0, 2, 3] = 1
        K[0, 3, 2] = 1
        ...
        if self.config["custom_mode"] == "wonderjourney":
            ...
            camera = PerspectiveCameras(K=K, R=R, T=T, in_ndc=False, image_size=((576, 1024),), device=self.device)
        else:
            raise("no implementation for given custom_mode")
        return camera
    ...
    @torch.no_grad()
    def inpaint(self, rendered_image, inpaint_mask, fill_mask=None, fill_mode = 'cv2_telea', postmask_type_override = None):
        # set process resolution
        if self.config["custom_mode"] == "wonderjourney":
            process_width, process_height = self.config["inpainting_resolution"], self.config["inpainting_resolution"]
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            process_width, process_height = 512, 288
        else:
            raise("no implementation for given custom_mode")
        ...
        if self.config['use_postmask']:
            ...
            postmask_type = self.config['postmask_type']
            if postmask_type_override is not None:
                postmask_type = postmask_type_override

            if postmask_type == "average":
                mask_block_size = 8
            ...
            elif postmask_type == "dilation":
                dilated_mask = dilation(inpaint_mask, torch.ones(3, 3).to('cuda'))
                mask = (dilated_mask[0, 0].cpu().numpy() * 255).astype(np.uint8)
            else:
                raise("no implementation for your postmask_type")


        init_image = Image.fromarray(img).resize( (process_width, process_height), Image.NEAREST)
        mask_image = Image.fromarray(mask).resize( (process_width, process_height), Image.NEAREST)
        ...
        inpainted_image = (inpainted_image / 2 + 0.5).clamp(0, 1).to(torch.float32) #Note: the size of inpainted_image is (process_width,process_height)
        ...
        return {"inpainted_image": inpainted_image, "latent": inpainted_image_latents.float()}  #Note: the size of inpainted_image is (process_width,process_height)
    ...
    def update_images_and_masks(self, latent, inpaint_mask):
        ...
        if self.config["custom_mode"] == "wonderjourney":
            ...
            decoded_image = ToTensor()( ToPILImage()(decoded_image[0]).resize((1024,576), Image.NEAREST) ).unsqueeze(0).to("cuda") # resize 到576*1024
        else:
            raise("no implementation for given custom_mode")
...
class KeyframeGen(FrameSyn):
    ...
    def refine_disp_with_segments(self, kf_idx, background_depth_cutoff=1./7.):
        if self.config["skip_depth_refine"] == True:

            #just use original depth and disparity as the refined_depth and refined_disparity
            refined_disparity = self.disparities[kf_idx].squeeze().cpu().numpy()
            ...
            # compute fake sky_mask, background_but_not_sky_mask
            sky_mask = torch.zeros_like(refined_depth) 
            background_but_not_sky_mask = torch.zeros_like(refined_depth) 
        ...
        ToPILImage()(sky_mask.float()).save(self.run_dir / 'images' / f"kf{kf_idx+1}_sky_mask_beforeErode.png") #M for monitoring
    ...
    def render(self, epoch):
        ...
        if self.config["custom_mode"] == "wonderjourney":
            current_camera = convert_pytorch3d_kornia(self.current_camera, self.config["init_focal_length"], h_size = 512, w_size = 512)
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            current_camera = convert_pytorch3d_kornia(self.current_camera, self.config["init_focal_length"], h_size = 576, w_size = 1024)
        else:
            raise("no implementation for given custom_mode")
        ...
        depth_normalizer = self.background_hard_depth #COMMENT: useless
        min_ratio = self.config['point_size_min_ratio'] #COMMENT: useless
        radius = self.config['point_size'] * (min_ratio + (1 - min_ratio) * (point_depth.permute([1, 0]) / depth_normalizer)) #COMMENT: useless
        radius = radius.clamp(max=self.config['point_size']*self.config['sky_point_size_multiplier']) #COMMENT: useless

        if self.config["custom_mode"] == "wonderjourney":
            height_width = (512,512)
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            height_width = (576,1024)
        else:
            raise("no implementation for given custom_mode")
        ...
        rendered_depth = renderer(point_cloud_orig_depth) + self.kf_delta_t  # kf1_estimate under kf2 frame, rendered #COMMENT: useless
        rendered_depth = rearrange(rendered_depth, "b h w c -> b c h w") #COMMENT: useless
...
class KeyframeInterp(FrameSyn):
    def __init__(self, config, inpainter_pipeline, depth_model, vae, rotation, 
        ...
        if config["custom_mode"] == "wonderjourney":
            ...
            x = torch.arange(1024 * kf2_upsample_coef)
            y = torch.arange(576 * kf2_upsample_coef)
        else:
            raise("no implementation for given custom_mode")
        ...
        if self.config["custom_mode"] == "wonderjourney":
            kf1_camera = convert_pytorch3d_kornia(kf1_camera, self.config["init_focal_length"], h_size = 512, w_size = 512)
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            kf1_camera = convert_pytorch3d_kornia(kf1_camera, self.config["init_focal_length"], h_size = 576, w_size = 1024)
        else:
            raise("no implementation for given custom_mode")
    ...
    def upsample_kf2(self):
        if self.config["custom_mode"] == "wonderjourney":
            kf2_h_size = 512 * self.kf2_upsample_coef
            kf2_w_size = 512 * self.kf2_upsample_coef
        elif self.config["custom_mode"] == "wonderjourney_576_1024":
            kf2_h_size = 576 * self.kf2_upsample_coef
            kf2_w_size = 1024 * self.kf2_upsample_coef
        else:
            raise("no implementation for given custom_mode")
        kf2_focal = self.config["init_focal_length"] * self.kf2_upsample_coef
        kf2_camera_upsample = convert_pytorch3d_kornia(self.kf2_camera, kf2_focal, h_size = kf2_h_size, w_size = kf2_w_size)
        kf2_depth_upsample = F.interpolate(self.kf2_depth, size=(kf2_h_size, kf2_w_size), mode="nearest")
        kf2_mask_upsample = F.interpolate(self.kf2_mask, size=(kf2_h_size, kf2_w_size), mode="nearest")
        kf2_pil_upsample = ToPILImage()(self.kf2_image[0]).resize((kf2_w_size, kf2_h_size), resample=Image.LANCZOS)
    ...
    def update_additional_point_cloud(self, rendered_depth, image, valid_mask=None, camera=None, points_2d=None, append_depth=False):
        ...
        current_camera = convert_pytorch3d_kornia(self.current_camera, self.config["init_focal_length"], h_size, w_size) if camera is None else camera
...
def convert_pytorch3d_kornia(camera, focal_length, h_size, w_size):
    ...
    extrinsics[:, :3, :3] = R
    extrinsics[:, :3, 3] = T
    h = torch.tensor([h_size], device="cuda")
    w = torch.tensor([w_size], device="cuda")
    K = torch.eye(4)[None].to("cuda")
    K[0, 0, 2] = w_size // 2  # 设置光心坐标的 x 值为图像宽度的一半  
    K[0, 1, 2] = h_size // 2  # 设置光心坐标的 y 值为图像高度的一半  
