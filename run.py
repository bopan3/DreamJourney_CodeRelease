...
from util.utils import load_example_yaml, merge_frames, merge_keyframes, KeyFrameDynamic, svd_inpainting
from util.segment_utils import create_mask_generator

import subprocess
import os

# NEW_INTERPOLATION_MODE = None
NEW_INTERPOLATION_MODE = "svd_inpainting"
DYNAMIC_MODE = "KeyFrameDynamic_EasyAnimate"

# DYNAMIC_MODE = "KeyFrameDynamic_cogVideoXFun"
# DYNAMIC_MODE = None
# DYNAMIC_MODE = "KeyFrameDynamic_1024_DynCraft"
# DYNAMIC_MODE = "KeyFrameDynamic_1024_svd"

  
# set network for fast download
...
def evaluate_for_svd_inpainting(model, epoch, num_frame, vmax=None):
    save_root = Path(model.run_dir) / "images"
    save_root.mkdir(exist_ok=True, parents=True)

    (save_root / "frames_for_svd_inpainting").mkdir(exist_ok=True, parents=True)
    (save_root / "masks_for_svd_inpainting").mkdir(exist_ok=True, parents=True)

    ToPILImage()(model.images[epoch][0]).save(save_root / "frames_for_svd_inpainting" / f"{epoch}.png")
    ToPILImage()(model.masks[epoch][0]).save(save_root / "masks_for_svd_inpainting" / f"{epoch}.png")
...
def run(config):
    ...
    if config["custom_mode"] == "wonderjourney":
        start_keyframe = Image.open(yaml_data['image_filepath']).convert('RGB').resize((512, 512))
    elif config["custom_mode"] == "wonderjourney_576_1024":
        start_keyframe = Image.open(yaml_data['image_filepath']).convert('RGB').resize((1024, 576))
    else:
        raise("no implementation for given custom_mode")

    # enforce control_text to be None
    content_prompt, style_prompt, adaptive_negative_prompt, background_prompt, control_text = yaml_data['content_prompt'], yaml_data['style_prompt'], yaml_data['negative_prompt'], yaml_data.get('background', None),  None
    ...
    for i in range(config['num_scenes']):
        ...
        if True in [True, "dynamic objects"]:
            inpainting_prompt = pt_gen.generate_prompt(style=style_prompt, entities=scene_dict['entities'], background=None, scene_name=scene_dict['scene_name'])
            pruned_entities = pt_gen.indentify_objects(scene_dict['entities'])
            inpainting_entities = ', '.join(pruned_entities)
        else:
            ...
            inpainting_entities = ', '.join(scene_dict['entities'])
        for j in range(config['num_keyframes']):
            ...
            if config['skip_gen']:
                ...
                for regen_id in range(config['regenerate_times'] + 1):
                    ...
                    # if config['c'] and regen_id <= config['regenerate_times'] -1:
                    if regen_id <= config['regenerate_times'] -1:
                        # pt_gen.dev_evaluate_image()
                        gpt_border, gpt_blur = pt_gen.evaluate_image_v2(ToPILImage()(inpaint_output['inpainted_image'][0]), eval_blur=False) #COMMENT: GPT4V evalute
                        # gpt_border, gpt_blur = pt_gen.human_evaluate_image(ToPILImage()(inpaint_output['inpainted_image'][0]), eval_blur=False) #COMMENT: human evalute for debugging
                        ...
                        regenerate = False
                    # regenerate, gpt_border, gpt_blur = False, False, False
                ...
                if config["finetune_decoder_gen"]: #M temperaraly commented out for acceleration
                    ToPILImage()(inpaint_output["inpainted_image"].detach()[0]).save(save_root / 'kf2_before_ft.png')
                    finetune_decoder(config, kf_gen, render_output, inpaint_output, config["custom_mode"], config['num_finetune_decoder_steps'])
                ...
                if config["finetune_depth_model"]: #M temperaraly commented out for acceleration
            ...
            if config["skip_visibility_check"]:
                kf2_depth_updated = kf2_depth_upsample
            else:
            ...
            if config["custom_mode"] == "wonderjourney":
                ...
                kf_interp.depths[0] = F.interpolate(kf2_depth_updated, size=(576, 1024), mode="nearest")
            else:
                raise("no implementation for given custom_mode")
            ...
            if NEW_INTERPOLATION_MODE is not None:
                if NEW_INTERPOLATION_MODE == "svd_inpainting":
                    kf_interp_for_svd_inpainting = deepcopy(kf_interp)
                    for epoch in tqdm(range(1, total_frames + 1)):
                        render_output_kf1 = kf_interp_for_svd_inpainting.render_kf1(epoch)

                        kf_interp_for_svd_inpainting.images.append(render_output_kf1["rendered_image"])
                        kf_interp_for_svd_inpainting.masks.append(render_output_kf1["inpaint_mask"])
                        evaluate_for_svd_inpainting(kf_interp_for_svd_inpainting, epoch, config["frames"], vmax=cutoff_depth*0.95)
                        empty_cache()
                        # add kf1 at the end for svd inpainting
                        if epoch == total_frames:
                            # save frame and mask for kf1 and kf2 in the end
                            ToPILImage()(kf2_image[0]).save(kf_interp_for_svd_inpainting.run_dir / "images" / "frames_for_svd_inpainting" / ( str(0) + ".png")) 
                            ToPILImage()(kf1_image[0]).save(kf_interp_for_svd_inpainting.run_dir / "images" / "frames_for_svd_inpainting" / f"{total_frames+1}.png")
                            ToPILImage()(torch.zeros_like(kf_interp_for_svd_inpainting.masks[epoch][0])).save(kf_interp_for_svd_inpainting.run_dir / "images" / "masks_for_svd_inpainting" / ( str(0) + ".png"))
                            ToPILImage()(torch.zeros_like(kf_interp_for_svd_inpainting.masks[epoch][0])).save(kf_interp_for_svd_inpainting.run_dir / "images" / "masks_for_svd_inpainting" / f"{total_frames+1}.png")
                            # last_keyframe = svd_inpainting(kf_interp_for_svd_inpainting.run_dir, config["custom_mode"])
                            # start_keyframe = last_keyframe
                            # all_keyframes[-1] = start_keyframe
    
            # for epoch in tqdm(range(1, total_frames + 1)):
            #     render_output_kf1 = kf_interp.render_kf1(epoch)

            #     inpaint_output = kf_interp.inpaint(render_output_kf1["rendered_image"], render_output_kf1["inpaint_mask"] , postmask_type_override = "average")
            ...
            if DYNAMIC_MODE is not None:
                if DYNAMIC_MODE == "KeyFrameDynamic_1024_DynCraft" or DYNAMIC_MODE == "KeyFrameDynamic_1024_svd":
                    keyframeId = i*config['num_keyframes'] + j
                    save_dir = Path(config['runs_dir']) / f"{keyframeId}_kf_dynamic"
                    start_keyframe = KeyFrameDynamic(start_keyframe, save_dir, custom_mode = config["custom_mode"], dynamic_mode = DYNAMIC_MODE, inpainting_entities = inpainting_entities)
                    all_keyframes[-1] = start_keyframe
                elif DYNAMIC_MODE == "KeyFrameDynamic_EasyAnimate" or DYNAMIC_MODE == "KeyFrameDynamic_cogVideoXFun":
                    keyframeId = i*config['num_keyframes'] + j
                    save_dir = Path(config['runs_dir']) / f"{keyframeId}_kf_dynamic"
                    start_keyframe = KeyFrameDynamic(start_keyframe, save_dir, custom_mode = config["custom_mode"], dynamic_mode = DYNAMIC_MODE, inpainting_entities = inpainting_entities)
                    all_keyframes[-1] = start_keyframe             
    ...
    #         KeyFrameDynamic(all_keyframes, save_dir, save_folder='dynamic_keyframes', fps=10, resolution_mode = "256_256")

    dt_string = datetime.now().strftime("%d-%m_%H-%M-%S")
    save_dir = Path(config['runs_dir']) / f"{dt_string}_merged"
    if not config['skip_interp']:
        merge_frames(all_rundir, save_dir=save_dir, fps=config["save_fps"], is_forward=False, save_depth=False, save_gif=False, DYNAMIC_MODE = "KeyFrameDynamic_1024_DynCraft")
