...
class EulerDiscreteSchedulerOutput(BaseOutput):
    ...
    pred_original_sample1: Optional[torch.FloatTensor] = None
    pred_original_sample2: Optional[torch.FloatTensor] = None
...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_single(
        ...
        if temp_cond_latents is not None:
            ...
            index_list = index_list[0:cond_len-1]
    ...
    def step_single_TRF(
        self,
        model_output: torch.FloatTensor,
        model_output_seqFlip: torch.FloatTensor,
        timestep: Union[float, torch.FloatTensor],
        sample: torch.FloatTensor,
        temp_cond_latents: Optional[torch.FloatTensor] = None,
        mask: Optional[torch.FloatTensor] = None,
        ...
        if self.config.prediction_type == "original_sample" or self.config.prediction_type == "sample":
            ...
            # denoised = model_output * c_out + input * c_skip
            pred_original_sample1 = model_output * (-sigma / (sigma**2 + 1) ** 0.5) + (sample / (sigma**2 + 1))
            pred_original_sample2 = torch.flip(model_output_seqFlip, dims=[1]) * (-sigma / (sigma**2 + 1) ** 0.5) + (sample / (sigma**2 + 1))

            # 创建从 0 到 1 的线性间隔  
            num_frame = model_output.shape[1]
            ...
            for i in range(num_frame):  
                ...
                pred_original_sample[:, i, :, :, :] =  (1 - weight) * pred_original_sample1[:, i, :, :, :] + weight * pred_original_sample2[:, i, :, :, :] 
        else:
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`, or `v_prediction`"
            )
        ...
        if compute_grad:
            ...
            return EulerDiscreteSchedulerOutput(prev_sample=prev_sample, pred_original_sample=pred_original_sample, pred_original_sample1 = pred_original_sample1, pred_original_sample2 = pred_original_sample2)
